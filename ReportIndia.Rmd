---
title: "Climate Change - Time Series Analysis(India)"
subtitle: "DV Project"
...
```{r,echo = T}
model2<-garchFit(~garch(1,1), data=avg_time, trace=F)
summary(model2)
predict(model2, n.ahead = 10, trace = FALSE)
```

```{r,echo = T}
model3<-garchFit(formula = ~arma(3,1,1)+garch(1,1), data=avg_time, trace=F)
summary(model3)
```
